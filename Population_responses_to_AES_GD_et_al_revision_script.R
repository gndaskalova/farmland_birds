...
farm_theme <- function(){
  theme_bw()+
    theme(axis.text.x=element_text(size=14),
          ...
          plot.title = element_text(size=20, vjust=0.5, hjust=0),
...
birds_lmo <- read.csv("birds_lmo_updated.csv")
summary(birds_lmo)
...
# Model with year mean centered and a yearcenter*treatment interaction
# Linnet
...
# Turning the categories for the management options into proportions
# of overall farm area
LI_lmo$uc <- as.numeric((LI_lmo$uc.area/LI_lmo$area)*100)
LI_lmo$mg.all <-as.numeric((LI_lmo$mg.all.area/LI_lmo$area)*100)
LI_lmo$ogwgw <-as.numeric((LI_lmo$ogwgw.area/LI_lmo$area)*100)
LI_lmo$cmsrg <-as.numeric((LI_lmo$cmsrg.area/LI_lmo$area)*100)
LI_lmo$mhm <-as.numeric((LI_lmo$mhm.area/LI_lmo$area)*100)
LI_lmo$mwet.all <-as.numeric((LI_lmo$mwet.all.area/LI_lmo$area)*100)
LI_lmo$wmar <-as.numeric((LI_lmo$wmar.area/LI_lmo$area)*100)
LI_lmo$mhedge.all <-as.numeric((LI_lmo$mhedge.all.area/LI_lmo$area)*100)
LI_lmo$gmarbeet <-as.numeric((LI_lmo$gmarbeet.area/LI_lmo$area)*100)

# Removing rows for which management option area info is not available
LI_lmo2 <- na.omit(LI_lmo)

# Linnet
LI_lmo_m2 <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                      random = ~farm + year + 
                        idv(uc+ mg.all + ogwgw + cmsrg + mwet.all + wmar + mhedge.all + gmarbeet),
                      pr = TRUE, nitt = 600000, data = LI_lmo2, family = "poisson", 
...
plot(LI_lmo_m2$Sol[,43:50])

# Calculating mean blup and 95% HPD for each treatment
ucTRUE <- mean(LI_lmo_m2$Sol[,43])
ucTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,43])
mg.allTRUE <- mean(LI_lmo_m2$Sol[,44])
mg.allTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,44])
ogwgwTRUE <- mean(LI_lmo_m2$Sol[,45])
ogwgwTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,45])
cmsrgTRUE <- mean(LI_lmo_m2$Sol[,46])
cmsrgTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,46])
mwet.allTRUE <- mean(LI_lmo_m2$Sol[,47])
mwet.allTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,47])
wmarTRUE <- mean(LI_lmo_m2$Sol[,48])
wmarTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,48])
mhedge.allTRUE <- mean(LI_lmo_m2$Sol[,49])
mhedge.allTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,49])
gmarbeetTRUE <- mean(LI_lmo_m2$Sol[,50])
gmarbeetTRUE_CI <- HPDinterval(LI_lmo_m2$Sol[,50])

# Combining the results into one data frame
LI_BLUPS <- data.frame(matrix("NA", nrow = 8, ncol = 5))
...
LI_BLUPS$BLUP <- as.numeric(c(ucTRUE, mg.allTRUE, ogwgwTRUE, cmsrgTRUE,
                              mwet.allTRUE, wmarTRUE, mhedge.allTRUE,
                              gmarbeetTRUE))
LI_BLUPS$upper <- as.numeric(c(ucTRUE_CI[,2], mg.allTRUE_CI[,2],
                               ogwgwTRUE_CI[,2], cmsrgTRUE_CI[,2],
...
LI_BLUPS$lower <- as.numeric(c(ucTRUE_CI[,1], mg.allTRUE_CI[,1],
                               ogwgwTRUE_CI[,1], cmsrgTRUE_CI[,1], 
                               mwet.allTRUE_CI[,1], wmarTRUE_CI [,1], 
                               mhedge.allTRUE_CI[,1], gmarbeetTRUE_CI[,1]))

# Reed bunting
RB_lmo$uc <- as.numeric((RB_lmo$uc.area/RB_lmo$area)*100)
RB_lmo$mg.all <-as.numeric((RB_lmo$mg.all.area/RB_lmo$area)*100)
RB_lmo$ogwgw <-as.numeric((RB_lmo$ogwgw.area/RB_lmo$area)*100)
RB_lmo$cmsrg <-as.numeric((RB_lmo$cmsrg.area/RB_lmo$area)*100)
RB_lmo$mwet.all <-as.numeric((RB_lmo$mwet.all.area/RB_lmo$area)*100)
RB_lmo$wmar <-as.numeric((RB_lmo$wmar.area/RB_lmo$area)*100)
RB_lmo$mhedge.all <-as.numeric((RB_lmo$mhedge.all.area/RB_lmo$area)*100)
RB_lmo$gmarbeet <-as.numeric((RB_lmo$gmarbeet.area/RB_lmo$area)*100)

# Remove NAs
RB_lmo2 <- na.omit(RB_lmo)

RB_lmo_m2 <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                      random = ~farm + year + 
                        idv(uc + mg.all + ogwgw + cmsrg + mwet.all + wmar + mhedge.all + gmarbeet),
                      pr = TRUE, nitt = 600000, data = RB_lmo2, family = "poisson", 
                      prior = prior3, burnin = 20000)

summary(RB_lmo_m2)

colnames(RB_lmo_m2$Sol)
plot(RB_lmo_m2$Sol[,43:50])

# Calculating mean blup and 95% HPD for each treatment
ucTRUE <- (mean(RB_lmo_m2$Sol[,43]))
ucTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,43]))
mg.allTRUE <- (mean(RB_lmo_m2$Sol[,44]))
mg.allTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,44]))
ogwgwTRUE <- (mean(RB_lmo_m2$Sol[,45]))
ogwgwTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,45]))
cmsrgTRUE <- (mean(RB_lmo_m2$Sol[,46]))
cmsrgTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,46]))
mwet.allTRUE <- (mean(RB_lmo_m2$Sol[,47]))
mwet.allTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,47]))
wmarTRUE <- (mean(RB_lmo_m2$Sol[,48]))
wmarTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,48]))
mhedge.allTRUE <- (mean(RB_lmo_m2$Sol[,49]))
mhedge.allTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,49]))
gmarbeetTRUE <- (mean(RB_lmo_m2$Sol[,50]))
gmarbeetTRUE_CI <- (HPDinterval(RB_lmo_m2$Sol[,50]))

# Combining the results into one data frame
RB_BLUPS <- data.frame(matrix("NA", nrow = 8, ncol = 5))
...
# Tree sparrow
TS_lmo$uc<-as.numeric((TS_lmo$uc.area/TS_lmo$area)*100)
TS_lmo$mg.all <-as.numeric((TS_lmo$mg.all.area/TS_lmo$area)*100)
TS_lmo$ogwgw <-as.numeric((TS_lmo$ogwgw.area/TS_lmo$area)*100)
TS_lmo$cmsrg <-as.numeric((TS_lmo$cmsrg.area/TS_lmo$area)*100)
TS_lmo$mwet.all <-as.numeric((TS_lmo$mwet.all.area/TS_lmo$area)*100)
TS_lmo$wmar <-as.numeric((TS_lmo$wmar.area/TS_lmo$area)*100)
TS_lmo$mhedge.all <-as.numeric((TS_lmo$mhedge.all.area/TS_lmo$area)*100)
TS_lmo$gmarbeet <-as.numeric((TS_lmo$gmarbeet.area/TS_lmo$area)*100)

# Remove NAs
TS_lmo2 <- na.omit(TS_lmo)

TS_lmo_m2 <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                      random = ~farm + year + 
                        idv(uc + mg.all + ogwgw + cmsrg + mwet.all + wmar + mhedge.all + gmarbeet),
                      pr = TRUE, nitt = 600000, data = TS_lmo2, family = "poisson", 
                      prior = prior3, burnin = 20000)

summary(TS_lmo_m2)

colnames(TS_lmo_m2$Sol)
plot(TS_lmo_m2$Sol[,43:50])

# Calculating mean blup and 95% HPD for each treatment
ucTRUE <- (mean(TS_lmo_m2$Sol[,43]))
ucTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,43]))
mg.allTRUE <- (mean(TS_lmo_m2$Sol[,44]))
mg.allTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,44]))
ogwgwTRUE <- (mean(TS_lmo_m2$Sol[,45]))
ogwgwTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,45]))
cmsrgTRUE <- (mean(TS_lmo_m2$Sol[,46]))
cmsrgTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,46]))
mwet.allTRUE <- (mean(TS_lmo_m2$Sol[,47]))
mwet.allTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,47]))
wmarTRUE <- (mean(TS_lmo_m2$Sol[,48]))
wmarTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,48]))
mhedge.allTRUE <- (mean(TS_lmo_m2$Sol[,49]))
mhedge.allTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,49]))
gmarbeetTRUE <- (mean(TS_lmo_m2$Sol[,50]))
gmarbeetTRUE_CI <- (HPDinterval(TS_lmo_m2$Sol[,50]))

# Combining the results into one data frame
TS_BLUPS <- data.frame(matrix("NA", nrow = 8, ncol = 5))
...
# Skylark
S_lmo$uc <- as.numeric((S_lmo$uc.area/S_lmo$area)*100)
S_lmo$mg.all <-as.numeric((S_lmo$mg.all.area/S_lmo$area)*100)
S_lmo$ogwgw <-as.numeric((S_lmo$ogwgw.area/S_lmo$area)*100)
S_lmo$cmsrg <-as.numeric((S_lmo$cmsrg.area/S_lmo$area)*100)
S_lmo$mwet.all <-as.numeric((S_lmo$mwet.all.area/S_lmo$area)*100)
S_lmo$wmar <-as.numeric((S_lmo$wmar.area/S_lmo$area)*100)
S_lmo$mhedge.all <-as.numeric((S_lmo$mhedge.all.area/S_lmo$area)*100)
S_lmo$gmarbeet <-as.numeric((S_lmo$gmarbeet.area/S_lmo$area)*100)

# Remova NAs
S_lmo2 <- na.omit(S_lmo)

S_lmo_m2 <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                     random = ~farm + year + 
                       idv(uc+ mg.all + ogwgw + cmsrg + mwet.all + wmar + mhedge.all + gmarbeet),
                     pr = TRUE, nitt = 600000, data = S_lmo2, family = "poisson", 
                     prior = prior3, burnin = 20000)

summary(S_lmo_m2)

colnames(S_lmo_m2$Sol)
plot(S_lmo_m2$Sol[,43:50])

# Calculating mean blup and 95% HPD for each treatment
ucTRUE <- (mean(S_lmo_m2$Sol[,43]))
ucTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,43]))
mg.allTRUE <- (mean(S_lmo_m2$Sol[,44]))
mg.allTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,44]))
ogwgwTRUE <- (mean(S_lmo_m2$Sol[,45]))
ogwgwTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,45]))
cmsrgTRUE <- (mean(S_lmo_m2$Sol[,46]))
cmsrgTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,46]))
mwet.allTRUE <- (mean(S_lmo_m2$Sol[,47]))
mwet.allTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,47]))
wmarTRUE <- (mean(S_lmo_m2$Sol[,48]))
wmarTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,48]))
mhedge.allTRUE <- (mean(S_lmo_m2$Sol[,49]))
mhedge.allTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,49]))
gmarbeetTRUE <- (mean(S_lmo_m2$Sol[,50]))
gmarbeetTRUE_CI <- (HPDinterval(S_lmo_m2$Sol[,50]))

# Combining the results into one data frame
S_BLUPS <- data.frame(matrix("NA", nrow = 8, ncol = 5))
...
# Yellowhammer
Y_lmo$uc <- as.numeric((Y_lmo$uc.area/Y_lmo$area)*100)
Y_lmo$mg.all <-as.numeric((Y_lmo$mg.all.area/Y_lmo$area)*100)
Y_lmo$ogwgw <-as.numeric((Y_lmo$ogwgw.area/Y_lmo$area)*100)
Y_lmo$cmsrg <-as.numeric((Y_lmo$cmsrg.area/Y_lmo$area)*100)
Y_lmo$mwet.all <-as.numeric((Y_lmo$mwet.all.area/Y_lmo$area)*100)
Y_lmo$wmar <-as.numeric((Y_lmo$wmar.area/Y_lmo$area)*100)
Y_lmo$mhedge.all <-as.numeric((Y_lmo$mhedge.all.area/Y_lmo$area)*100)
Y_lmo$gmarbeet <-as.numeric((Y_lmo$gmarbeet.area/Y_lmo$area)*100)

# Remove NAs
Y_lmo2 <- na.omit(Y_lmo)

Y_lmo_m2 <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                     random = ~farm + year + 
                       idv(uc + mg.all + ogwgw + cmsrg + mwet.all + wmar + mhedge.all + gmarbeet),
                     pr = TRUE, nitt = 600000, data = Y_lmo2, family = "poisson", 
                     prior = prior3, burnin = 20000)

summary(Y_lmo_m2)

colnames(Y_lmo_m2$Sol)
plot(Y_lmo_m2$Sol[,43:50])

# Calculating mean blup and 95% HPD for each treatment
ucTRUE <- (mean(Y_lmo_m2$Sol[,43]))
ucTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,43]))
mg.allTRUE <- (mean(Y_lmo_m2$Sol[,44]))
mg.allTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,44]))
ogwgwTRUE <- (mean(Y_lmo_m2$Sol[,45]))
ogwgwTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,45]))
cmsrgTRUE <- (mean(Y_lmo_m2$Sol[,46]))
cmsrgTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,46]))
mwet.allTRUE <- (mean(Y_lmo_m2$Sol[,47]))
mwet.allTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,47]))
wmarTRUE <- (mean(Y_lmo_m2$Sol[,48]))
wmarTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,48]))
mhedge.allTRUE <- (mean(Y_lmo_m2$Sol[,49]))
mhedge.allTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,49]))
gmarbeetTRUE <- (mean(Y_lmo_m2$Sol[,50]))
gmarbeetTRUE_CI <- (HPDinterval(Y_lmo_m2$Sol[,50]))

# Combining the results into one data frame
Y_BLUPS <- data.frame(matrix("NA", nrow = 8, ncol = 5))
...
(eff_plot4 <- ggplot(eff_size4, aes(x = species, y = post.mean)) +
    ...
    geom_point(shape = 1, size = 4,colour = "black", position = position_dodge(.3)) +
    farm_theme() +
    labs(x = "", y = "Coefficient\n", title = "b) AES model coefficients\n") +
...
(AES_years_plot <- ggplot(AESyears_eff, aes(x = species, y = slope)) +
    ...
    geom_point(shape = 1, size = 4,colour = "black", position = position_dodge(.3)) +
    farm_theme() +
    labs(x = "", y = "Coefficient\n", title = "c) AES duration model coefficients\n") +
    geom_hline(yintercept = 0, linetype = "dashed") +
    theme(legend.text = element_text(size = 12, face = "bold"),    
          legend.title = element_blank(),                        
          legend.position = c(0.9, 0.9),
...
ggsave(panel, file="eff_size_panel_updated.png", width = 8, height = 12)
...
(Yraw <- ggplot(Ysum, aes(x = year, y = abundance, 
    ...
    geom_point(shape = 1, size = 4,colour = "black") +
    farm_theme() +
    labs(y = "Abundance\n", title = "e) Yellowhammer\n") +
    scale_x_continuous(breaks = seq(2003, 2015, 3)) +
    scale_y_continuous(limits = c(5, 23), breaks = c(5, 10, 15, 20)) +
    scale_colour_manual(values = c("#EE7600", "#00868B")) + 
    theme(axis.title.x = element_blank(), legend.position = "none"))

yearly_panel_raw <- grid.arrange(LIraw, Sraw, RBraw, Yraw, TSraw, ncol = 1)
ggsave(yearly_panel_raw, file="yearly_panel_new.png", width = 6, height = 18)
...
lmo_panel3 <- grid.arrange(LI_BLUP_p, RB_BLUP_p, TS_BLUP_p, 
                           S_BLUP_p, Y_BLUP_p, ncol = 1)
ggsave(lmo_panel3, file = "lmoBLUPs_updated.png", width = 6, height = 20)

# Get sample size for land management options
sample_sizes <- Y_lmo2 %>% filter(treatment == "AES") %>%
  dplyr::select(farm, uc, mg.all, ogwgw, cmsrg, mwet.all, 
         wmar, mhedge.all, gmarbeet) %>%
  gather(lmo, prop.area, select = c(2:9)) %>% distinct()

# uc - 32
sample_sizes %>% filter(lmo == "uc") %>% filter(prop.area != 0)

# mg.all - 9
sample_sizes %>% filter(lmo == "mg.all") %>% filter(prop.area != 0)

# ogwgw - 18
sample_sizes %>% filter(lmo == "ogwgw") %>% filter(prop.area != 0)

# cmsrg - 18
sample_sizes %>% filter(lmo == "cmsrg") %>% filter(prop.area != 0)

# mwet.all - 21
sample_sizes %>% filter(lmo == "mwet.all") %>% filter(prop.area != 0)

# wmar - 30
sample_sizes %>% filter(lmo == "wmar") %>% filter(prop.area != 0)

# mhedge.all - 42
sample_sizes %>% filter(lmo == "mhedge.all") %>% filter(prop.area != 0)

# gmarbeet - 35
sample_sizes %>% filter(lmo == "gmarbeet") %>% filter(prop.area != 0)
...
# Before/After AES analysis for the farms that switches treatment ----
ba_farms <- birds_aes %>% filter(farm %in% c("Mains of Kindrought", "Todhills",
                                      "Blackpots", "Cunninghaugh",
                                      "Dallachy", "Ironhill", "Ewebrae",
                                      "Mains of Dudwich", "Minnonie",
                                      "Paddocklaw", "Wellheads", 
                                      "St John's Wells", "Upper Blairmund",
                                      "Waulkmill", "Slackadale",
                                      "Newton of Mountblairy"))
write.csv(ba_farms, file = "ba_farms.csv")
ba_farms <- read.csv("ba_farms.csv")
ba_farms <- filter(ba_farms, timing %in% c("after", "before"))

# Linnet
ba_farms_LI <- filter(ba_farms, species == "LI")
ba_farms_LI$latcenter <- ba_farms_LI$latitude - mean(ba_farms_LI$latitude)
ba_farms_LI$visits_f <- as.factor(ba_farms_LI$visits)
ba_farms_LI$loncenter <- ba_farms_LI$longitude - mean(ba_farms_LI$longitude)
ba_farms_LI$areacenter <- log(ba_farms_LI$area) - mean(log(ba_farms_LI$area))

ba_LI <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                 random = ~farm + year, pr = TRUE, nitt = 200000, singular.ok = T,
                 data = ba_farms_LI, family = "poisson", prior = prior2, burnin = 20000)
summary(ba_LI)

# Reed bunting
ba_farms_RB <- filter(ba_farms, species == "RB")
ba_farms_RB$latcenter <- ba_farms_RB$latitude - mean(ba_farms_RB$latitude)
ba_farms_RB$visits_f <- as.factor(ba_farms_RB$visits)
ba_farms_RB$loncenter <- ba_farms_RB$longitude - mean(ba_farms_RB$longitude)
ba_farms_RB$areacenter <- log(ba_farms_RB$area) - mean(log(ba_farms_RB$area))

ba_RB <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                  random = ~farm + year, pr = TRUE, nitt = 200000, 
                  data = ba_farms_RB, family = "poisson", prior = prior2, burnin = 20000)
summary(ba_RB)

# Tree sparrow
ba_farms_TS <- filter(ba_farms, species == "TS")
ba_farms_TS$latcenter <- ba_farms_TS$latitude - mean(ba_farms_TS$latitude)
ba_farms_TS$visits_f <- as.factor(ba_farms_TS$visits)
ba_farms_TS$loncenter <- ba_farms_TS$longitude - mean(ba_farms_TS$longitude)
ba_farms_TS$areacenter <- log(ba_farms_TS$area) - mean(log(ba_farms_TS$area))

ba_TS <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                  random = ~farm + year, pr = TRUE, nitt = 200000, 
                  data = ba_farms_TS, family = "poisson", prior = prior2, burnin = 20000)
summary(ba_TS)

# Skylark
ba_farms_S <- filter(ba_farms, species == "S")
ba_farms_S$latcenter <- ba_farms_S$latitude - mean(ba_farms_S$latitude)
ba_farms_S$visits_f <- as.factor(ba_farms_S$visits)
ba_farms_S$loncenter <- ba_farms_S$longitude - mean(ba_farms_S$longitude)
ba_farms_S$areacenter <- log(ba_farms_S$area) - mean(log(ba_farms_S$area))

ba_S <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                  random = ~farm + year, pr = TRUE, nitt = 200000, 
                  data = ba_farms_S, family = "poisson", prior = prior2, burnin = 20000)
summary(ba_S)

# Yellowhammer
ba_farms_Y <- filter(ba_farms, species == "Y")
ba_farms_Y$latcenter <- ba_farms_Y$latitude - mean(ba_farms_Y$latitude)
ba_farms_Y$visits_f <- as.factor(ba_farms_Y$visits)
ba_farms_Y$loncenter <- ba_farms_Y$longitude - mean(ba_farms_Y$longitude)
ba_farms_Y$areacenter <- log(ba_farms_Y$area) - mean(log(ba_farms_Y$area))

ba_Y <- MCMCglmm(abundance ~ treatment + areacenter + latcenter + loncenter + visits_f, 
                  random = ~farm + year, pr = TRUE, nitt = 200000, 
                  data = ba_farms_Y, family = "poisson", prior = prior2, burnin = 20000)
summary(ba_Y)

# Model tabels
LI_ba_sum <- clean.MCMC(ba_LI)
stargazer(LI_ba_sum, type = "html", summary = FALSE, digits = 2)
RB_ba_sum <- clean.MCMC(ba_RB)
stargazer(RB_ba_sum, type = "html", summary = FALSE, digits = 2)
TS_ba_sum <- clean.MCMC(ba_TS)
stargazer(TS_ba_sum, type = "html", summary = FALSE, digits = 2)
S_ba_sum <- clean.MCMC(ba_S)
stargazer(S_ba_sum, type = "html", summary = FALSE, digits = 2)
Y_ba_sum <- clean.MCMC(ba_Y)
stargazer(Y_ba_sum, type = "html", summary = FALSE, digits = 2)


# Excluding the ones which were AES and then control
ba_farms2 <- filter(ba_farms, farm != "Blackpots" | year != "2015")
ba_farms2 <- filter(ba_farms2, farm != "Cunninghaugh" | year != "2015")
ba_farms2 <- filter(ba_farms2, farm != "Ironhill" | year != "2015")
ba_farms2 <- filter(ba_farms2, farm != "Ewebrae" | year != "2015")
ba_farms2 <- filter(ba_farms2, farm != "Minnonie" | year != "2015")
ba_farms2 <- filter(ba_farms2, farm != "Paddocklaw" | year != "2003")
ba_farms2 <- filter(ba_farms2, farm != "Paddocklaw" | year != "2004")
ba_farms2 <- filter(ba_farms2, farm != "Paddocklaw" | year != "2006")
ba_farms2 <- filter(ba_farms2, farm != "Paddocklaw" | year != "2008")
ba_farms2 <- filter(ba_farms2, farm != "Wellheads" | year != "2003")
ba_farms2 <- filter(ba_farms2, farm != "Wellheads" | year != "2004")
ba_farms2 <- filter(ba_farms2, farm != "Wellheads" | year != "2006")
ba_farms2 <- filter(ba_farms2, farm != "Waulkmill" | year != "2003")
ba_farms2 <- filter(ba_farms2, farm != "Waulkmill" | year != "2004")
ba_farms2 <- filter(ba_farms2, farm != "Waulkmill" | year != "2006")

ba_farms2 <- filter(ba_farms2, farm != "Todhills" |
                      farm != "Slackadale" | farm != "Newton of Mountblairy")

# Linnet
ba_farms_LI <- filter(ba_farms2, species == "LI")
ba_farms_LI$latcenter <- ba_farms_LI$latitude - mean(ba_farms_LI$latitude)
ba_farms_LI$visits_f <- as.factor(ba_farms_LI$visits)
ba_farms_LI$loncenter <- ba_farms_LI$longitude - mean(ba_farms_LI$longitude)
ba_farms_LI$areacenter <- log(ba_farms_LI$area) - mean(log(ba_farms_LI$area))
...
# Reed bunting
ba_farms_RB <- filter(ba_farms2, species == "RB")
ba_farms_RB$latcenter <- ba_farms_RB$latitude - mean(ba_farms_RB$latitude)
ba_farms_RB$visits_f <- as.factor(ba_farms_RB$visits)
ba_farms_RB$loncenter <- ba_farms_RB$longitude - mean(ba_farms_RB$longitude)
ba_farms_RB$areacenter <- log(ba_farms_RB$area) - mean(log(ba_farms_RB$area))
...
# Tree sparrow
ba_farms_TS <- filter(ba_farms2, species == "TS")
ba_farms_TS$latcenter <- ba_farms_TS$latitude - mean(ba_farms_TS$latitude)
ba_farms_TS$visits_f <- as.factor(ba_farms_TS$visits)
ba_farms_TS$loncenter <- ba_farms_TS$longitude - mean(ba_farms_TS$longitude)
ba_farms_TS$areacenter <- log(ba_farms_TS$area) - mean(log(ba_farms_TS$area))
...
# Skylark
ba_farms_S <- filter(ba_farms2, species == "S")
ba_farms_S$latcenter <- ba_farms_S$latitude - mean(ba_farms_S$latitude)
ba_farms_S$visits_f <- as.factor(ba_farms_S$visits)
ba_farms_S$loncenter <- ba_farms_S$longitude - mean(ba_farms_S$longitude)
ba_farms_S$areacenter <- log(ba_farms_S$area) - mean(log(ba_farms_S$area))
...
# Yellowhammer
ba_farms_Y <- filter(ba_farms2, species == "Y")
ba_farms_Y$latcenter <- ba_farms_Y$latitude - mean(ba_farms_Y$latitude)
ba_farms_Y$visits_f <- as.factor(ba_farms_Y$visits)
ba_farms_Y$loncenter <- ba_farms_Y$longitude - mean(ba_farms_Y$longitude)
ba_farms_Y$areacenter <- log(ba_farms_Y$area) - mean(log(ba_farms_Y$area))
